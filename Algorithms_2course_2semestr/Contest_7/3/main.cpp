...
int Wagner_Fischer_for_Damerau_Levenshtein(string& s, string& t)
{
    ...
    for (int i = 1; i < s.size() + 1; ++i)
    {
        for (int j = 1; j < t.size() + 1; ++j)
        {
            ...
            if (s[i - 2] == t[j - 1] && s[i - 1] == t[j - 2] && (i != 1 && j != 1)) {
                if (s[i - 1] == t[j - 1]) {
                    d[i][j] = min(min((d[i - 1][j] + 1), (d[i][j - 1] + 1)), min((d[i - 1][j - 1]), (d[i - 2][j - 2] + 1)));
                } else {
                    d[i][j] = min(min((d[i - 1][j] + 1), (d[i][j - 1] + 1)), min((d[i - 1][j - 1] + 1), (d[i - 2][j - 2] + 1)));
                }
            } else {
                if (s[i - 1] == t[j - 1]) {
